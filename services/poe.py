...
GREETING_RE = re.compile(
    r"^\s*(?:и\s+вам\s+|ещё\s+раз\s+|еще\s+раз\s+)?"
    r"(?:здравствуйте|здрасьте|доброго\s+дня|добрый\s+день|доброе\s+утро"
    r"|добрый\s+вечер|приветствую|привет)"
    r"(?:\s+(?:вас|ещё\s+раз|еще\s+раз|снова))?\s*[!,.;:)…-]+\s*",
    re.I,
)
